create function stockai_private.update_internal_code(p_item uuid,p_code text,p_expected text)
...
revoke all on function stockai_private.update_internal_code(uuid,text,text) from public,anon;
grant execute on function stockai_private.update_internal_code(uuid,text,text) to authenticated;

create or replace function public.stockai_update_internal_code(p_item uuid,p_code text,p_expected text)
returns void language sql security invoker set search_path='' as $$
 select stockai_private.update_internal_code(p_item,p_code,p_expected);
$$;
